		...
		<div class="app-contain">
			...
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button v-if="btnAuth('orders'+(scope.row.status?'/'+scope.row.status:''),'核销')" type="warning" @click="writeOffClick(scope.row)">核销</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	//核销
	const writeOffClick = (row)=>{
		ElMessageBox.confirm(`确定核销该订单？`, '提示', {
			confirmButtonText: '是',
			cancelButtonText: '否',
			type: 'warning',
		}).then(()=>{
